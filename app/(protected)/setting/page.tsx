...
import TextInput from '@/app/ui/inputs/TextInput/TextInput';
...
export default function Account() {
  ...
  // Функция для проверки соответствия менеджера
  const checkForMatchingManager = useCallback((managers: any[], userEmail: string) => {
    ...
    if (manager) {
      ...
      fetchPartnersForManager(manager.partners);
    }
  }, []);
  ...
  // Функция для получения партнеров
  ...
  // Эффект для получения менеджеров
  useEffect(() => {
  ...
  // Проверка статуса загрузки и сессии
      ...
      <div>
        ...
        {matchedManager ? (
          <div>
            ...
            <p>Телефон: {matchedManager.phone}</p>
            {/* Отображение дополнительных данных менеджера */}
          </div>
        ) : (
          <>
