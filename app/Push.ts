...
//     case 'denied':
//       console.warn('Notifications are denied');
//       alert('Чтобы получать уведомления, разрешите их в настройках браузера.');
//       break;

//     case 'granted':
//       // Если разрешение уже предоставлено, просто подписываем
//       registerAndSubscribe(onSubscribe);
//       break;
...
//           registerAndSubscribe(onSubscribe);
//         } else {
//           console.warn('Notifications are denied after request');
//           alert('Чтобы получать уведомления, разрешите их в настройках браузера.');
//         }
//       });
//       break;
...
export function checkPermissionStateAndAct(
  ...
  const state = Notification.permission; // Теперь это просто строка
...
// Подписка на уведомления
async function subscribe(onSubscribe: (subscription: PushSubscription | null) => void): Promise<void> {
  try {
    const registration = await navigator.serviceWorker.ready;
    const applicationServerKey = urlBase64ToUint8Array(process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY!);
    const subscription = await registration.pushManager.subscribe({
